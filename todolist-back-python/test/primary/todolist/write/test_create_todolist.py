...
def test_create_todolist(client: TestClient, use_case: MagicMock, app: FastAPI, fake: TodolistFaker):
    todolist = fake.a_todolist()
    response = client.post(f"/todolist/{todolist.to_key()}", json={"name": todolist.to_name()})

    assert response.status_code == 200
    assert response.json() is None
    use_case.execute.assert_called_with(todolist_key=todolist.to_key(), todolist_name=todolist.to_name())
