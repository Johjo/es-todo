...
def test_convert_many_tasks(fake: TodolistFaker):
    task_1 = fake.a_task().having(name=fake.task_name(), is_open=TaskOpen(True)).to_presentation()
    task_2 = fake.a_task().having(name=fake.task_name(), is_open=TaskOpen(True)).to_presentation()
