...
def test_get_by_when_todolist_has_tasks(sut: TodolistSetPeewee, fake: TodolistFaker):
    expected_todolist = replace(fake.a_todolist(), tasks=[fake.a_task(), fake.a_task()])
    feed_todolist(replace(fake.a_todolist(), tasks=[fake.a_task(), fake.a_task()]))
    feed_todolist(expected_todolist)
...
def test_update_todolist(sut: TodolistSetPeewee, fake: TodolistFaker):
    initial_todolist = replace(fake.a_todolist(), tasks=[fake.a_task(), fake.a_task()])
    sut.save_snapshot(initial_todolist)

    expected_todolist = replace(initial_todolist, tasks=[fake.a_task(), fake.a_task()])
    sut.save_snapshot(expected_todolist)
