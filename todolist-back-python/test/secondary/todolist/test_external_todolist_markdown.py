...
import pytest
from expression import Some, Nothing, pipe
...
def test_ignore_empty_lines():
    ...
    assert sut.all_tasks() == [TaskImported(name="line 1", is_open=True, execution_date=Nothing), TaskImported(name="line 2", is_open=True, execution_date=Nothing)]
...
def test_read_many_task_from_markdown(fake: TodolistFaker, line_separator):
    expected_tasks = [imported_task_from(fake.a_task()), imported_task_from(fake.a_task()), imported_task_from(fake.a_task())]
...
@pytest.mark.parametrize("text, expected", [
    ["- [ ] l{execution_date=2022-07-13}",
     TaskImported(name=TaskName("l"), is_open=TaskOpen(True), execution_date=Some(TaskExecutionDate(date(2022, 7, 13))))],
    ["- [ ] le{execution_date=2022-07-13}",
     TaskImported(name=TaskName("le"), is_open=TaskOpen(True), execution_date=Some(TaskExecutionDate(date(2022, 7, 13))))],
    ["- [ ] left{execution_date=2022-07-13}",
     TaskImported(name=TaskName("left"), is_open=TaskOpen(True), execution_date=Some(TaskExecutionDate(date(2022, 7, 13))))],
    ...
    ["- [ ] left{execution_date=2022-07-13}right",
     TaskImported(name=TaskName("left right"), is_open=TaskOpen(True), execution_date=Some(TaskExecutionDate(date(2022, 7, 13))))],
])
